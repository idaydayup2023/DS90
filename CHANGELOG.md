...
- 其他通用修复：
  - **并发稳定性**：修复了 `StateStore` (SQLite) 在多线程环境下因连接共享导致的 `SQLite objects created in a thread can only be used in that same thread` 错误。
  - **网络健壮性**：将 FTP 默认超时时间从 300 秒进一步增加到 3600 秒（1 小时），以支持 20GB 以上超大文件在跨分区迁移时的长耗时操作。
