...
程序优先选择同目录英文 SRT，其次尝试 ffmpeg 内嵌字幕，再按配置调用 PGS OCR/ASR worker。默认使用 `translategemma:12b` 顺序翻译：每批最多 80 条、8000 字符，并携带前后各 6 条只读上下文；严格响应校验失败后自动拆为 40/20 条继续，而不是并发争抢同一个 Ollama 模型。全部初译完成后再对完整字幕做一次一致性校对，只修改模型明确返回的人名、术语、称谓或语气不一致项。完整校对超过 `consistency_max_chars` 或返回结构不合格时安全停止，不会跳过校对直接发布。

翻译和校对响应都必须是索引合法、无重复且无多余字段的 JSON；发布前会复核字幕数量、时间轴、非空文本、目标文字比例和序列化结果。输出为 `<视频名>.ai.srt`，同时写入带源文件哈希、提示协议和全部翻译参数的 manifest；批次、上下文、模型或校对配置改变都会使旧缓存失效，只有 `--force` 会显式绕过仍然有效的缓存。
