...
def _prompt(dir_path: str, moved_files: list[str]) -> str:
    ...
    return (
        ...
        "We migrated a movie/TV file out of a source directory. Now the directory appears empty.\n"
        "Decide whether it is SAFE to delete the EMPTY directory.\n"
        "Rules:\n"
        "1) Only output JSON. No explanations outside JSON.\n"
        "2) If you are not very sure, choose \"unknown\".\n"
        "3) Choose \"delete\" ONLY when the directory is clearly a movie/TV download folder (e.g. includes title/year/resolution/group) or a season/episode folder.\n"
        "4) Choose \"keep\" for generic folders (Downloads, Movies, TV, torrents, incomplete, temp) or anything that may hold other downloads.\n"
        "5) When in doubt: unknown.\n\n"
...
def _prompt_residual(dir_path: str, moved_files: list[str], residual_files: list[tuple[str, int | None]]) -> str:
    moved = "\n".join(f"- {x}" for x in moved_files[:20])
    if len(moved_files) > 20:
        moved += f"\n- ... ({len(moved_files) - 20} more)"
    ...
    return (
        "You are a cautious cleanup agent for a media migration tool.\n"
        "We migrated a movie/TV file out of a source directory. The directory has residual files left behind.\n"
        "Decide whether it is SAFE to delete these residual files AND then delete the directory.\n"
        "Rules:\n"
        "1) Only output JSON. No explanations outside JSON.\n"
        "2) If you are not very sure, choose \"unknown\".\n"
        "3) Choose \"delete\" ONLY when the directory is clearly a movie/TV download folder AND the residual files are typical extras (nfo/images/text/sample) that are safe to remove.\n"
        "4) Choose \"keep\" for generic folders (Downloads, Movies, TV, torrents, incomplete, temp) or anything that may hold other downloads.\n"
        "5) When in doubt: unknown.\n\n"
        f"DIRECTORY: {dir_path}\n"
        "MOVED FILES (examples):\n"
...
def _llm_decide_empty_dir(cfg: AppConfig, plan: MovePlan, dir_path: str) -> CleanupDecision:
    ...
    text = ollama.generate(model=cfg.ollama.model, prompt=_prompt(dir_path, moved_files), temperature=0.0).text
...
def _llm_decide_residual_files(cfg: AppConfig, plan: MovePlan, dir_path: str, residual_files: list[tuple[str, int | None]]) -> CleanupDecision:
    ollama = OllamaMcp(cfg.ollama.base_url, timeout_seconds=cfg.ollama.timeout_seconds)
    moved_files = [plan.source.video_path, *plan.source.subtitle_paths]
    text = ollama.generate(model=cfg.ollama.model, prompt=_prompt_residual(dir_path, moved_files, residual_files), temperature=0.0).text
    obj = _extract_json(text)
    decision = str(obj.get("decision") or "unknown").strip().lower()
    confidence = obj.get("confidence")
    try:
...
def cleanup_source_residual_dirs(cfg: AppConfig, source_storage: StorageMcp, plan: MovePlan) -> None:
    ...
    while cur and cur != "/":
        ...
        dirs = [(p, sz) for p, t, sz in entries if t == "dir"]
        files = [(p, sz) for p, t, sz in entries if t == "file"]
        if dirs:
            return

        if files and cfg.cleanup.delete_residual_files:
            video_exts = {e.lower() for e in cfg.video.extensions}
            for p, _sz in files:
                name = posixpath.basename(p).lower()
                _stem, ext = posixpath.splitext(name)
                if ext in video_exts and "sample" not in name:
                    return

            decision = _llm_decide_residual_files(cfg, plan, cur, files)
        ...
        if not source_storage.list_dir(cur):
            decision = _llm_decide_empty_dir(cfg, plan, cur)
            if decision.decision != "delete":
                log.info("cleanup keep dir=%s decision=%s confidence=%s reason=%s", cur, decision.decision, decision.confidence, decision.reason)
                return
            if decision.confidence is not None and decision.confidence < cfg.cleanup.min_confidence:
