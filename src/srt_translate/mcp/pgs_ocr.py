...
class PgsOcrMcp:
    ...
    def track_to_srt(
        ...
        with tempfile.TemporaryDirectory(prefix="srt_translate_pgs_ocr_") as td:
            ...
            cmd.append(str(sup))
            env = os.environ.copy()
            p = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, check=False, env=env)
            if p.returncode != 0:
                raise RuntimeError((p.stderr or p.stdout or "").strip() or f"pgsrip exit={p.returncode}")
            srts = sorted(tmp.glob("*.srt"), key=lambda x: x.stat().st_size, reverse=True)
            if not srts:
                raise RuntimeError("pgsrip produced no srt")
